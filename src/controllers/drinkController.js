...
module.exports = {
    ...
    postCreateDrinksAPI: async (req, res) => {
        try {
    ...
    putUpdateDrinksAPI: async (req, res) => {
        try {
            ...
            const result = await Drink.updateOne(
                { _id: currentdDrinkId },
                { name: newDrinkName, price: newDrinkPrice },
            );
            return res.status(200).json({
                errorCode: 0,
                data: result,
